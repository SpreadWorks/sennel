...
function fixture(stepId = "draft", {
  ...
  coordinator.createRequest = (input) => {
    const conditionalDraft = ["draft-refine", "draft-gate-repair"]
      .includes(input.invocation?.action?.nextAction?.step);
    ...
    if (conditionalDraft && input.deferPreparation !== true) {
      ...
      flowManager.claimDraftStepExecution({
        ...
        executionClaim: new DraftWorkerExecutionClaim({
          dispatchInvocationId: handoff.dispatchInvocationId,
    ...
    if (handoff?.policy.kind === "source") {
      coordinator.startSourceWorker({ ctx: input.ctx, request: handoff, invocation: input.invocation });
      SOURCE_REQUEST_LIFECYCLES.set(handoff, { coordinator, ctx: input.ctx });
    }
    return handoff;
  };
...
function candidateDraftQuestion({ id = "q1", revision = 0, question = "Which public behavior should be selected?" } = {}) {
  return {
...
function seal(request) {
  if (request.policy.kind === "source") {
    ...
    const sealed = sealParentMaterializedSourceWorkerEffect({ request, now: () => new Date("2026-08-04T00:00:01.000Z") });
    const lifecycle = SOURCE_REQUEST_LIFECYCLES.get(request);
    if (lifecycle !== undefined) lifecycle.coordinator.finishSourceWorker({ ctx: lifecycle.ctx, request });
    return sealed;
  }
...
function rejectedSourcePlan(request, error, { workerStopped = true } = {}) {
  return resolveSourceHandoffTransitionPlan({
...
describe("worker artifact handoff", () => {
  ...
  it("confirms a source handoff after regular flow set metric docsRead commands", () => {
    ...
    try {
      ...
      assert.equal(result.canonicalObservationAdvance.kind, "source-worker-canonical-observation-advance");
      assert.deepEqual(
        result.canonicalObservationAdvance.addedActivityIds.filter((id) => id.startsWith("metric-recorded-")),
        value.flowManager.activityLedger(value.specId)
          .filter((activity) => activity.transition.operation === "record_metric")
  ...
  it("cleans a rejected source handoff so a retry observes only its own diff", () => {
    ...
    try {
      ...
      assert.ok(rejection instanceof WorkerArtifactHandoffError
        && rejection.code === "FLOW_SOURCE_HANDOFF_EFFECT_MUTATION_INVALID");
      const plan = rejectedSourcePlan(request, rejection);
      assert.equal(plan.disposition, "rollback");
      assert.ok(value.coordinator.recordSourceFailure({ ctx: value.ctx, request, plan }));
  ...
  it("fails closed without cleaning a rejected source handoff after index mutation", () => {
    ...
    try {
      ...
      const plan = rejectedSourcePlan(request, new WorkerArtifactHandoffError(
        "invalid", "FLOW_SOURCE_HANDOFF_FINALIZE_AUTHORITY_VIOLATION", "source was staged before worker exit",
      ), { workerStopped: false });
      assert.equal(plan.disposition, "quarantine");
      assert.equal(value.coordinator.rollbackRejectedSourceHandoff({
        ctx: value.ctx, request, mutationAuthority: authority, plan,
      }), false);
      assert.equal(fs.existsSync(request.directory), true);
      assert.equal(fs.readFileSync(path.join(value.mainRoot, "product.js"), "utf8"), "export const value = 2;\n");
    } finally {
  ...
  it("discards a parent-validated artifact handoff after a pre-commit interruption", () => {
    ...
    try {
      ...
      assert.throws(
        () => commitDraftResult(crashing, {
          ctx: value.ctx,
          request,
          preparation,
          stepResult: new DraftCreatedResult(),
        }),
        (error) => error instanceof WorkerArtifactHandoffError
  ...
  it("derives durable Task file effects from each Attempt manifest across inherited dirt, shared changes, deletion, and no-change", () => {
    ...
    const completeTaskImplementation = (taskId, mutate, noChangeReason = null) => {
      ...
      coordinator.startSourceWorker({ ctx: context(), request, invocation: invocation(taskId) });
      const authority = WorkerArtifactMutationAuthoritySnapshot.capture(request);
      mutate();
      const effect = materializeSourceWorkerEffect({
        request,
      ...
      sealParentMaterializedSourceWorkerEffect({ request });
      coordinator.finishSourceWorker({ ctx: context(), request });
  ...
  it("recovers a checkpoint-bound sealed source handoff through a fresh manager", () => {
    const value = fixture("implement", { specRecord: validSpec() });
    try {
      initializeGitRepository(value);
      const request = value.coordinator.createRequest({
      ...
      fs.writeFileSync(request.payloadPath("effects.json"), json(implementationEffect(request, ["product.js"]).toJSON()));
      seal(request);
      const recoveredManager = new FlowManager({
        root: value.executionRoot,
        mainRoot: value.mainRoot,
        inWorktree: true,
        specId: value.specId,
      });
      const recovered = new WorkerArtifactHandoffCoordinator().recoverPending({
        ctx: { ...value.ctx, flowManager: recoveredManager },
      });
      assert.equal(recovered.completed, true);
      assert.equal(findStepById(recoveredManager.load(value.specId).steps, "implement").status, "done");
    } finally {
      removeTmpDir(value.mainRoot);
    }
  });
  ...
  it("publishes a sealed draft and completes the step only in the parent transaction", () => {
    ...
    try {
      ...
      const preparation = value.coordinator.prepareDraftWorker({ ctx: value.ctx, request });
      const result = commitDraftResult(value.coordinator, {
        ctx: value.ctx,
        request,
        preparation,
        stepResult: new DraftCreatedResult(),
  ...
  it("keeps non-conditional Draft workers on their existing Connector commit path", async () => {
    const value = fixture("draft");
    try {
      const request = value.coordinator.createRequest({
        ctx: value.ctx,
      ...
      const expectedDraft = draftDocument("non-conditional dispatcher path");
      fs.writeFileSync(request.payloadPath("draft.json"), json(expectedDraft));
      seal(request);
      const preparation = value.coordinator.prepareDraftWorker({ ctx: value.ctx, request });

      const completed = await new RunDispatchCommand({ handoffCoordinator: value.coordinator })
        .runDraftWorkerStep(
          value.ctx,
          request,
      ...
      assert.equal(completed.stepResult.kind, "draft-created");
  ...
  it("rejects direct Draft reconcile without a Step-selected Result", () => {
    const value = fixture("draft");
    try {
      const request = value.coordinator.createRequest({
        ctx: value.ctx,
      ...
      assert.throws(
        () => value.coordinator.reconcile({ ctx: value.ctx, request }),
        (error) => error instanceof WorkerArtifactHandoffError
          && error.code === "FLOW_DRAFT_STEP_RESULT_REQUIRED",
      );
  ...
  it("promotes a sealed draft-refine Candidate, retains the Attempt, and records an idempotent receipt", async () => {
    ...
    try {
      ...
      const completed = commitDraftResult(value.coordinator, {
        ctx: value.ctx,
        request,
        preparation,
        stepResult: new DraftRefineAwaitingAnswerResult(),
      });
      ...
      assert.equal(completed.receipt.id, activity.result.draftSettlementReceipt.id);
      assert.equal(completed.receipt.publicationDigest, activity.result.draftSettlementReceipt.publicationDigest);
      ...
      assert.equal(value.flowManager.activityLedger(value.specId).length, waitingActivities);
      assert.equal(value.flowManager.activityLedger(value.specId).at(-1).result.draftSettlementReceipt.id,
        activity.result.draftSettlementReceipt.id);

      const replay = commitDraftResult(value.coordinator, {
        ctx: value.ctx,
        request,
        preparation,
        stepResult: new DraftRefineAwaitingAnswerResult(),
      ...
      assert.equal(replay.receipt.id, activity.result.draftSettlementReceipt.id);
      assert.equal(value.flowManager.activityLedger(value.specId).length, beforeActivities + 2);
  ...
  it("persists the existing draft-refine Await Result once and replays it without duplication", async () => {
    ...
    const value = fixture("draft-refine", {
      beforeActivate(fixtureValue) {
        publishDraftBeforeTarget(fixtureValue, awaiting);
      },
    });
    try {
      const binding = await new DraftRefineConnector({
        flowManager: value.flowManager,
        specId: value.specId,
      }).connect();
      ...
      const afterFirst = value.flowManager.activityLedger(value.specId);
      const firstActivity = afterFirst.at(-1);
      assert.equal(first.type, STEP_RESULT_TYPE.USER_INPUT_REQUIRED);
      assert.equal(afterFirst.length, before + 1);
      assert.equal(firstActivity.transition.operation, "record_draft_step_settlement");
      assert.equal(firstActivity.result.stepResult.kind, "draft-refine-awaiting-answer");
      assert.equal(firstActivity.result.draftSettlementReceipt.settlementKind, "await");

      const second = await step.execute();
      const afterReplay = value.flowManager.activityLedger(value.specId);
      assert.equal(second.type, STEP_RESULT_TYPE.USER_INPUT_REQUIRED);
      assert.equal(afterReplay.length, before + 1);
      assert.equal(afterReplay.at(-1).result.draftSettlementReceipt.id,
        firstActivity.result.draftSettlementReceipt.id);
    } finally {
      removeTmpDir(value.mainRoot);
    }
  });

  it("rejects a changed sealed Draft payload instead of accepting the committed handoff as a replay", () => {
    const value = fixture();
    try {
      const request = value.coordinator.createRequest({
        ctx: value.ctx,
      ...
      fs.writeFileSync(request.payloadPath("draft.json"), json(draftDocument("original sealed draft")));
      seal(request);
      const preparation = value.coordinator.prepareDraftWorker({ ctx: value.ctx, request });
      const binding = new DraftWorkerStepBinding({ request });
      ...
      const interrupted = new WorkerArtifactHandoffCoordinator({
        faultInjector({ phase }) {
          if (phase === "before-worker-handoff-cleanup-rename") throw new Error("retain sealed handoff for replay check");
        },
      });
      assert.throws(
      ...
      fs.writeFileSync(request.payloadPath("draft.json"), json(draftDocument("changed after settlement")));
      assert.throws(
  ...
  it("admits and claims a conditional Draft worker before materializing its request", async () => {
    const value = fixture("draft-refine", {
      autoApprove: true,
      beforeActivate(fixtureValue) {
        publishDraftBeforeTarget(fixtureValue, draftWithQuestionLedger([candidateDraftQuestion()]));
      },
    });
    try {
      const state = value.flowManager.load();
      ...
      assert.deepEqual(
        value.flowManager.canonicalState(value.specId).attempt.consumption.toJSON(),
        beforeAttempt.consumption.toJSON(),
      );
      assert.equal(value.coordinator.restoreClaimedDraftRequest({
        ctx: value.ctx,
        state: value.flowManager.load(),
  ...
  it("reaches the conditional Draft provider only after its exact generation is claimed", async () => {
    const value = fixture("draft-refine", {
      autoApprove: true,
      beforeActivate(fixtureValue) {
        publishDraftBeforeTarget(fixtureValue, draftWithQuestionLedger([candidateDraftQuestion()]));
    ...
    try {
      ...
      const action = session.captureAction(draftWorkerAction("draft-refine"), "conditional-draft-step");
      ...
      class SealingAgent extends Agent {
        ...
        async call(_prompt, options) {
          ...
          assert.equal(execution.lifecycle.phase, "claimed");
          assert.equal(execution.lifecycle.executionGeneration, 0);
          assert.equal(execution.lifecycle.claim.requestDigest, options.executionEnvironment.SENNEL_FLOW_DISPATCH_INVOCATION
            ? JSON.parse(options.executionEnvironment.SENNEL_FLOW_DISPATCH_INVOCATION).requestDigest
            : null);
          const payloadPath = requestDocument.payloads.find((entry) => entry.logicalName === "draft.json").payloadPath;
          fs.writeFileSync(payloadPath, json(draftWithQuestionLedger([])));
          sealWorkerArtifactHandoff({
            requestPath,
            invocationId: options.executionEnvironment.SENNEL_FLOW_DISPATCH_INVOCATION_ID,
          });
      ...
      const attempt = await dispatcher.runWorkerAttempt({
        ...value.ctx,
        flowState: value.flowManager.load(),
      }, invocation, null, new SealingAgent());
      assert.equal(attempt.error, null);
      assert.equal(attempt.stepResult.kind, "draft-refine-completed");
      const terminal = value.flowManager.canonicalState(value.specId)
  ...
  it("reenters the same sealed worker generation after publication interruption", async () => {
    ...
    try {
      ...
      const action = session.captureAction(draftWorkerAction("draft-refine"), "conditional-draft-recovery");
      const invocation = new FlowDispatchInvocation({
        session,
        action,
        authorization: new UnapprovedFlowDispatchAuthorization(action),
      ...
      class InterruptingCoordinator extends WorkerArtifactHandoffCoordinator {
        interrupted = false;
      ...
      const coordinator = new InterruptingCoordinator();
      const dispatcher = new RunDispatchCommand({ handoffCoordinator: coordinator });
      const ctx = { ...value.ctx, flowState: value.flowManager.load() };
      const conflictingSession = new FlowDispatchSession({ target: session.target });
      const conflictingAction = conflictingSession.captureAction(
        draftWorkerAction("draft-refine"),
        "conditional-draft-recovery",
      );
      const conflictingInvocation = new FlowDispatchInvocation({
        session: conflictingSession,
        action: conflictingAction,
        authorization: new UnapprovedFlowDispatchAuthorization(conflictingAction),
      });
      const conflictingRequest = coordinator.createRequest({
        ctx,
        state: value.flowManager.load(),
        invocation: conflictingInvocation,
        deferPreparation: true,
      });
      await assert.rejects(
        () => dispatcher.runWorkerAttempt(ctx, invocation, null, new SealingAgent()),
        /simulated interruption after publication/,
      );
      const binding = {
        runId: value.flowManager.canonicalState(value.specId).runId,
        specId: value.specId,
        stepId: "draft-refine",
        attempt: value.flowManager.canonicalState(value.specId).attempt,
      };
      const interruptedExecution = value.flowManager.draftStepExecutionState({ binding }).lifecycle;
      assert.equal(interruptedExecution.phase, "publication");

      conflictingRequest.prepare();
      fs.writeFileSync(conflictingRequest.payloadPath("draft.json"), json(draftWithQuestionLedger([])));
      sealWorkerArtifactHandoff({
        requestPath: conflictingRequest.requestPath,
        invocationId: conflictingRequest.dispatchInvocationId,
      });
      assert.throws(
        () => coordinator.prepareDraftWorker({ ctx, request: conflictingRequest }),
        (error) => error instanceof WorkerArtifactHandoffError
          && error.code === "FLOW_DRAFT_EXECUTION_CLAIM_MISMATCH",
      );

      const recovered = await dispatcher.runWorkerAttempt(
      ...
      assert.equal(recovered.stepResult.kind, "draft-refine-completed");
      assert.equal(providerCalls, 1);
      assert.equal(value.flowManager.canonicalState(value.specId)
        .findNode("draft-refine").result.draftSettlementReceipt.executionLifecycle.phase, "terminal");
    } finally {
      removeTmpDir(value.mainRoot);
    }
  });

  it("keeps auto-approved draft-refine Execution on the same Attempt", async () => {
    for (const { name, autoApprove, source } of [
      ...
      try {
        ...
        commitDraftResult(coordinator, {
          ...
          request,
          preparation,
          stepResult: new DraftRefineWorkerRequiredResult(),
        });
        const next = await new GetNextActionCommand().execute({
          ...value.ctx,
          specId: value.specId,
  ...
  it("settles the next auto-approved draft-refine Result on that retained Attempt", () => {
    const initial = draftWithQuestionLedger([candidateDraftQuestion()]);
    const completedDraft = draftWithQuestionLedger([]);
    const value = fixture("draft-refine", {
      autoApprove: true,
      beforeActivate(fixtureValue) {
        publishDraftBeforeTarget(fixtureValue, initial);
      },
    });
    try {
      const firstRequest = value.coordinator.createRequest({
        ctx: value.ctx,
        state: value.flowManager.load(),
        invocation: value.invocation,
      });
      fs.writeFileSync(firstRequest.payloadPath("draft.json"), json(initial));
      seal(firstRequest);
      const firstPreparation = value.coordinator.prepareDraftWorker({ ctx: value.ctx, request: firstRequest });
      const first = commitDraftResult(value.coordinator, {
        ctx: value.ctx,
        request: firstRequest,
        preparation: firstPreparation,
        stepResult: new DraftRefineWorkerRequiredResult(),
      });
      const retained = value.flowManager.canonicalState(value.specId).attempt;

      const secondRequest = value.coordinator.createRequest({
        ctx: value.ctx,
        state: value.flowManager.load(),
        invocation: { ...value.invocation, id: "dispatch-worker-handoff-next" },
      });
      fs.writeFileSync(secondRequest.payloadPath("draft.json"), json(completedDraft));
      seal(secondRequest);
      const second = commitDraftResult(value.coordinator, {
        ctx: value.ctx,
        request: secondRequest,
        preparation: value.coordinator.prepareDraftWorker({ ctx: value.ctx, request: secondRequest }),
        stepResult: new DraftRefineCompletedResult(),
      });

      assert.equal(first.receipt.binding.attemptId, retained.id);
      ...
      assert.equal(value.flowManager.canonicalState(value.specId).nextAction().nodeId, "draft-coverage-review");
    } finally {
      removeTmpDir(value.mainRoot);
    }
  });

  it("records each fresh Candidate promotion selected after the prior answer on its retained Attempt", async () => {
    const q1 = candidateDraftQuestion({ id: "q1" });
    const q2 = candidateDraftQuestion({ id: "q2", question: "Which compatibility boundary applies?" });
    const initial = draftWithQuestionLedger([q1, q2]);
    const value = fixture("draft-refine", {
      beforeActivate(fixtureValue) {
        publishDraftBeforeTarget(fixtureValue, initial);
      },
    ...
    try {
      ...
      fs.writeFileSync(firstRequest.payloadPath("draft.json"), json(initial));
      seal(firstRequest);
      const first = await executeSelectedWorker(firstRequest, value.coordinator.prepareDraftWorker({
        ctx: value.ctx, request: firstRequest,
      }));
      assert.equal(first.result.type, STEP_RESULT_TYPE.USER_INPUT_REQUIRED);
      const answer = new SetDraftAnswerCommand().execute({
        ...value.ctx,
        flowState: value.flowManager.load(),
        questionId: "q1",
        questionRevision: 1,
      ...
      const secondRequest = value.coordinator.createRequest({
        ctx: value.ctx,
        state: value.flowManager.load(),
        invocation: { ...value.invocation, id: "dispatch-worker-handoff-second-candidate" },
      });
      fs.writeFileSync(secondRequest.payloadPath("draft.json"), value.flowManager.readArtifact({
        specId: value.specId, logicalKey: "draft", consumerNodeId: "draft-refine",
      }).bytes);
      seal(secondRequest);
      const second = await executeSelectedWorker(secondRequest, value.coordinator.prepareDraftWorker({
        ctx: value.ctx, request: secondRequest,
      }));

      assert.equal(second.result.type, STEP_RESULT_TYPE.USER_INPUT_REQUIRED);
  ...
  it("retries one missing Draft handoff before persisting its terminal worker failure", async () => {
    ...
    try {
      ...
      assert.ok(failure);
      assert.equal(Object.hasOwn(failure.result, "stepResult"), false);
  ...
  it("runs the bound Draft Step through the existing handoff and advances to questions review", async () => {
    ...
    try {
      ...
      assert.equal(attempt.error, null);
      assert.equal(attempt.stepResult.type, STEP_RESULT_TYPE.COMPLETED);
      const reloaded = new FlowManager({
        root: value.executionRoot, mainRoot: value.mainRoot, inWorktree: true, specId: value.specId,
      });
