...
test("next-action refuses a Spec Gate publication without its atomic Result", async () => {
  ...
  try {
    ...
    await assert.rejects(
      new GetNextActionCommand().execute({
        root, mainRoot: root, executionRoot: root, specId,
        flowManager: manager, flowState: manager.load(specId),
      }),
      /Spec Gate publication lacks its atomic Step Result and Settlement/,
    );
...
test("published Spec Gate failure without Result stays refused after reload", async () => {
  ...
  try {
    ...
    await assert.rejects(new GetNextActionCommand().execute(ctx),
      /Spec Gate publication lacks its atomic Step Result and Settlement/);

    const reloaded = new FlowManager({ root, mainRoot: root, inWorktree: false, specId });
    await assert.rejects(new GetNextActionCommand().execute({
      ...ctx, flowManager: reloaded, flowState: reloaded.load(specId),
    }), /Spec Gate publication lacks its atomic Step Result and Settlement/);
    const before = {
    ...
    await assert.rejects(
      ...
      /Spec Gate result is already settled for the active Attempt/,
    );
    assert.equal(reloaded.artifactCatalog(specId).artifacts.find((artifact) => (
      artifact.logicalKey === "spec.gate"
