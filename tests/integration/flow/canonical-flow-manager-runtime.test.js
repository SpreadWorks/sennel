...
import {
  completeTaskStageHandoff,
  createTaskStageHandoff,
} from "../../support/builders/task-review-scenario.js";
...
/** Run a sealed Spec Triage or Repair handoff through the same Step runner as dispatch. */
async function settleSealedSpecReviewWorker({ ctx, coordinator, stepId, payload }) {
  const request = coordinator.createRequest({
    ctx,
    state: ctx.flowManager.load(ctx.specId),
    invocation: {
      id: `canonical-spec-review-${stepId}`,
      target: { digest: crypto.createHash("sha256").update(`target:${stepId}`).digest("hex") },
      action: {
        digest: crypto.createHash("sha256").update(`action:${stepId}`).digest("hex"),
...
function settleFixtureSpecReviewWorker(manager, specId, stepResult, { specRecord } = {}) {
  const state = manager.canonicalState(specId);
  const binding = {
    runId: state.runId, specId, stepId: stepResult.stepId, attempt: state.attempt,
  };
  ...
  manager.settleSpecStepResult({
    ...
    artifactBaselines: [new CanonicalFlowArtifactBaseline({
      logicalKey: "spec.review",
      parameters: { revision: current.review.identity.revision.toString() },
      digest: current.descriptor.hash,
      byteLength: current.descriptor.size,
    })],
  });
  return publication;
}
...
function request(specId = "001-canonical-manager", overrides = {}) {
  const suppliedSpec = overrides.specRecord?.toJSON?.() ?? overrides.specRecord ?? { ...emptySpecStub(), tasks: [] };
  const specRecord = new CurrentFlowSpecRecord({
    ...suppliedSpec,
    requirements: (suppliedSpec.requirements ?? []).map((requirement) => ({
      ...requirement,
      ...(requirement.task_ids === undefined ? { task_ids: ["T-fixture"] } : {}),
...
describe("FlowManager canonical Version-1 runtime", () => {
  ...
  it("routes normal manager step transitions and Task addition through the Activity Store", () => {
    ...
    assert.deepEqual(task.steps.map((step) => step.id), ["T-1-impl", "T-1-review", "T-1-triage", "T-1-repair", "T-1-gate"]);
    ...
    assert.equal(fs.existsSync(taskLocation.triageDirectory), true);
    assert.equal(fs.existsSync(taskLocation.repairDirectory), true);
    ...
    assert.deepEqual(
      ...
      ["T-1-impl", "T-1-review", "T-1-triage", "T-1-repair", "T-1-gate"],
    );
    assert.equal(fs.existsSync(taskLocation.implDirectory), true);
    assert.equal(fs.existsSync(taskLocation.reviewDirectory), true);
    assert.equal(fs.existsSync(taskLocation.triageDirectory), true);
  ...
  it("routes Task runtime metadata aliases to the active materialized child", () => {
    ...
    for (const [nodeId, sequence] of [["T-1-impl", 1], ["T-1-review", 2]]) {
  ...
  it("publishes a non-pass gate Attempt without a legacy result/source sibling", async () => {
    ...
    assert.deepEqual(facts.specCycle.toJSON(), { cycle: 1, completedRepairs: 0 });
  ...
  it("stops the fourth semantic Spec Gate cycle after three committed repairs", async () => {
    const repository = root();
    fs.writeFileSync(path.join(repository, "README.md"), "Spec cycle limit fixture\n");
    initGitRepo(repository);
    commitAll(repository, "initial Spec cycle limit fixture");
    const manager = new FlowManager({ root: repository, mainRoot: repository, inWorktree: false });
    const created = manager.createFresh(request("001-spec-gate-cycle-limit", {
      specRecord: new CurrentFlowSpecRecord({
        ...emptySpecStub(),
        requirements: [{
          id: "R-1", desc: "Bound repeated Spec repair cycles.", task_ids: ["T-1"], testable: false,
        }],
        tasks: [],
    ...
    activateNonBlockingPolicy({
      root: repository,
      flowManager: manager,
      reason: "Continue with explicit acceptance of the capped Spec Gate evidence.",
    });
    const advisory = decisionContextForActiveFlow(repository, manager.loadReadOnly(created.specId), manager);
    recordNonBlockingDecision({
      root: repository,
      flowManager: manager,
      choice: "continue",
      reason: "Continue after the bounded Spec Gate repair cycle is exhausted.",
      remainingRisk: "Approval retains the unresolved Spec Gate semantic findings.",
        ...
        effect: {
          ...
          triage: {
            ...
            dispositions: [{
              ...
              basis: scenario.disposition === "apply" ? "repair-required" : "finding-invalid",
    ...
    completeCanonicalSourceHandoff({
      ...
      effect: {
        ...
        triage: {
          version: 1,
          dispositions: [
            { findingKey: "repair-me", disposition: "apply", basis: "repair-required", rationale: "The recurring finding still requires repair." },
  ...
  it("records an explicit acceptance-risk decision without overwriting review evidence", async () => {
    ...
    assert.ok(issueLog.entries.some((entry) => entry.step === "acceptance-decision"));
  ...
  it("parks the canonical Flow after an explicit acceptance-risk abort", async () => {
    ...
    const issueLogDescriptor = manager.artifactCatalog(created.specId).artifacts
      .find((entry) => entry.logicalKey === "issue.log") ?? null;
    if (issueLogDescriptor !== null) {
      const issueLog = JSON.parse(manager.readArtifact({
        specId: created.specId,
        logicalKey: "issue.log",
        consumerNodeId: "final-regression",
      }).bytes.toString("utf8"));
      assert.equal(issueLog.entries.some((entry) => entry.step === "acceptance-decision"), false);
    }
  });
  ...
  it("delivers a linked-Issue report through the canonical catalog and report outbox", async () => {
    ...
    try {
      ...
      manager.addTask({
        id: "T-1",
        title: "Publish the linked report",
        goal: "Bind the implementation file map to the report.",
        origin: "plan",
        added_round: 0,
        status: "pending",
      }, { specId: created.specId });
  ...
  it("publishes a changed Spec repair through sealed workers and reads its revision at Spec Gate", async () => {
    const { repository, manager, created } = activeSpecReviewFixture("001-spec-review-repaired-revision");
    const findingId = "repair-canonical-goal";
    let providerCalls = 0;
    const review = new RunReviewCommand({
      resolveTreeSha: () => "a".repeat(40),
      resolveTargetStateDigest: () => "b".repeat(64),
      runCommand(_command, _args, options) {
        ...
        writeSpecReviewDeltaOutput(options, [{
          findingId,
          kind: "blocking",
          title: "Clarify the Spec goal",
          target: "goal",
          body: "The canonical Spec needs the reviewed goal wording.",
          issue: "The initial goal omits the reviewed publication outcome.",
          requiredChange: "Replace the root goal with the reviewed wording.",
          whyBlocking: "The repaired Spec must preserve the approved publication scope.",
        }]);
        ReviewWorkUnit.fromEnvironment(options.env).seal();
        return { ok: true, status: 0, stdout: "", stderr: "", signal: null, killed: false };
      },
    ...
    const result = await review.execute(reviewCtx);
    ...
    const ctx = {
      root: repository, mainRoot: repository, executionRoot: repository,
      specId: created.specId, flowManager: manager,
    };
    const coordinator = new WorkerArtifactHandoffCoordinator({
      now: () => new Date("2026-09-22T00:00:00.000Z"),
    });
    const activityCountBeforeWorkers = manager.activityLedger(created.specId).length;
    const triage = await settleSealedSpecReviewWorker({
      ctx, coordinator, stepId: "spec-triage",
      payload(request) {
        const canonicalReview = new CanonicalSpecReview(
          request.inputs.find((entry) => entry.name === "review.json").document,
        );
        return {
          version: 2,
          stage: "spec-triage",
          identity: canonicalReview.identity.toJSON(),
          baseReviewDigest: canonicalReview.digest,
          findings: [{
            findingId,
            disposition: "apply",
            evidence: "The root goal is an existing reviewed field.",
            allowedTargets: [{
              target: { entity: "spec", field: "goal" },
              operationKinds: ["replace-field"],
            }],
          }],
          operations: [],
        };
    ...
    const repair = await settleSealedSpecReviewWorker({
      ctx, coordinator, stepId: "spec-repair",
      payload(request) {
        const canonicalReview = new CanonicalSpecReview(
          request.inputs.find((entry) => entry.name === "review.json").document,
        );
        ...
        return {
          ...
          operations: [{
            ...
            kind: "replace-field",
            target: { entity: "spec", field: "goal" },
            expectedDigest: crypto.createHash("sha256").update(JSON.stringify(spec.goal)).digest("hex"),
            replacement: "Publish a revised canonical Spec after the original review.",
            reason: "Apply the reviewed root goal correction.",
          }],
        };
      },
    });
    assert.equal(repair.stepResult.kind, "spec-repair-changed");
    assert.equal(repair.receipt.targetStepId, "spec-gate");
    const after = currentSpecRevisionAuthority(manager, created.specId);
    assertSpecRevisionAdvanced(before, after);
    assert.equal(JSON.parse(after.root.toString("utf8")).goal, "Publish a revised canonical Spec after the original review.");
    const gateReview = manager.readCurrentSpecReview({
      specId: created.specId,
      consumerNodeId: "spec-gate",
    }).review;
    assert.equal(gateReview.identity.revision.value, after.revision);
    assert.equal(gateReview.identity.digest, crypto.createHash("sha256").update(after.root).digest("hex"));
    assert.equal(gateReview.findings.byId(findingId).disposition, "apply");
    assert.equal(gateReview.audit.at(-1).stage, "spec-repair");
    const activityCount = manager.activityLedger(created.specId).length;
    assert.equal(activityCount, activityCountAfterRepairActivation + 1);
    assert.deepEqual(
      manager.activityLedger(created.specId)
        .filter((activity) => ["spec-triage", "spec-repair"].includes(activity.nodeId)
          && activity.result?.draftSettlementReceipt !== undefined)
        .map((activity) => ({ nodeId: activity.nodeId, receiptId: activity.result.draftSettlementReceipt.id })),
      [
        { nodeId: "spec-triage", receiptId: triage.receipt.id },
        { nodeId: "spec-repair", receiptId: repair.receipt.id },
      ],
    );
    const reloaded = new FlowManager({ root: repository, mainRoot: repository, inWorktree: false });
    const reloadedReview = reloaded.readCurrentSpecReview({
      specId: created.specId,
      consumerNodeId: "spec-gate",
    }).review;
    const reloadedSpec = reloaded.readArtifact({
      specId: created.specId,
      logicalKey: "spec.record",
      consumerNodeId: "spec-gate",
    });
    assert.deepEqual(reloadedReview.identity.toJSON(), gateReview.identity.toJSON());
    assert.deepEqual(reloadedSpec.bytes, after.root);
    assert.equal(reloaded.canonicalState(created.specId).nextAction().nodeId, "spec-gate");
    assert.equal(reloaded.activityLedger(created.specId).length, activityCount);
    assert.equal(providerCalls, 1);
    assert.equal(manager.activityLedger(created.specId).length, activityCount);
  });
  ...
  it("records triage and repair review receipts in their confirmation Activities", () => {
    ...
    settleFixtureSpecReviewWorker(manager, created.specId, new SpecRepairUnchangedResult());
  ...
  it("rebases the merged spec-repair review onto exactly one changed Spec revision", () => {
    ...
    const repair = settleFixtureSpecReviewWorker(manager, created.specId, new SpecRepairChangedResult(), {
      specRecord: new CanonicalWorkerSpecPublication(document),
    });
    const after = currentSpecRevisionAuthority(manager, created.specId);
    assertSpecRevisionAdvanced(before, after);
  ...
  it("keeps an unresolved Task Review in triage and out of Acceptance deferral", async () => {
    const repository = root();
    initializeReviewSource(repository);
    ...
    const result = await publishTaskReview({
      repository,
      manager,
      specId,
      blockingFindings: [canonicalTaskReviewFinding({
        findingKey: "task-review-unresolved-finding",
  ...
  it("derives every cataloged test member finalization from durable test confirmations", () => {
    ...
    const confirmation = (id, attemptId, sequence, confirmedAt, confirmationOrder) => ({
      ...
      sequence,
      confirmationOrder,
      transition: { operation: "confirm_attempt", nodeId: "test-gate", status: "done" },
    ...
    const publication = (id, attemptId, sequence, confirmationOrder) => ({
      ...
      sequence,
      confirmationOrder,
      transition: { operation: "publish_artifacts", nodeId: "test-gate", status: null },
      result: null,
      timing: null,
    });
    const catalog = {
  ...
  it("preserves V1 identity, finalized state, Activities, and catalog references after a fresh read", () => {
    ...
    assert.equal(referencesBeforeFinalization.some((artifact) => artifact.logicalKey === "spec.record"), true);
  ...
  it("retains the current revision when a flow-level source worker does not change the Spec", () => {
    ...
    const created = manager.createFresh(request(specId, {
      specRecord: new CurrentFlowSpecRecord({
        ...
        requirements: [{
          id: "R1",
          desc: "Publish a validated artifact.",
          priority: "must",
          task_ids: ["T-1"],
          testable: false,
        }],
    ...
    manager.addTask({
      id: "T-1",
      title: "Publish the validated artifact",
      goal: "Publish a validated source worker artifact.",
      origin: "plan",
      added_round: 0,
      status: "pending",
    }, { specId: created.specId });
  ...
  it("settles a v2 Draft Gate StepResult and atomically records its applied worker outcome", async () => {
    ...
    assert.deepEqual(
      workerAction.context.planGateRepair.observations.map((observation) => observation.fingerprint),
      workerAction.context.planGateRepair.observationFingerprints,
    );
    ...
    assert.equal(repairCompletion.completed, true);
    const repairOutcome = manager.readArtifact({
      specId: created.specId,
      logicalKey: "plan.gate.repair.outcome",
      parameters: { repairId: canonicalRepair.idempotencyKey },
      consumerNodeId: "system",
    });
    assert.equal(JSON.parse(repairOutcome.bytes).disposition, "applied");
    assert.equal(repairOutcome.descriptor.activityId, manager.activityLedger(created.specId).at(-1).id);
  ...
  it("rejects a Task Gate decision when allow-listed source changes after result publication", async () => {
    ...
    const review = await publishTaskReview({ repository, manager, specId });
  ...
  it("routes the first current Task Gate receipt through the sealed repair plan without a raw retry", () => {
    ...
    assert.deepEqual(taskHistory.attempts.map((entry) => entry.attempt.value), [1]);
    assert.equal(taskHistory.current.payload.artifact.payload.result, "fail");
    assert.deepEqual(
  ...
  for (const repairedReviewStages of [false, true]) {
    ...
    initializeReviewSource(repository);
    const manager = new FlowManager({ root: repository, mainRoot: repository, inWorktree: false });
    const specId = `001-task-gate-two-round-convergence-${repairedReviewStages ? "completed" : "skipped"}`;
    new TaskLifecycleFixture({
      flowManager: manager,
      specId,
    ...
    const firstGateRepair = canonicalPlanGateRepairForTarget({
      flowManager: manager,
      state: manager.load(specId),
    ...
    confirmTaskImplementationMutation({
      ...
      gateRepair: {
        version: 1,
        summary: "Changed the current Task source for every first-round Gate observation.",
        results: firstGateRepair.observationRequests.map((request) => ({
          fingerprint: request.fingerprint.toString(),
    ...
    assert.deepEqual(implementations[1].budget.toJSON(), {
      ...
      gateAttemptSequenceAtStart: 1,
    });
    if (repairedReviewStages) {
      const recurringFinding = canonicalTaskReviewFinding({
        findingKey: "round-two-recurring-finding",
        title: "Round-two behavior remains incomplete",
        failureMode: "spec_behavior_contradiction",
        file: "README.md",
        requirementId: "R-T-1",
        guardrailId: null,
        issue: "The Task still lacks the round-two behavior.",
        suggestion: "Apply the exact round-two repair.",
        disposition: "must-fix",
        rationale: "R-T-1 requires the completed behavior.",
      });
      for (let ordinal = 1; ordinal <= 4; ordinal += 1) {
        const rejectedReview = await publishTaskReview({
          repository,
          manager,
          specId,
        ...
        const recurrence = repair.request.inputs
          .find((input) => input.name === "task-review-recurrence.json").document;
        fs.appendFileSync(path.join(repository, "README.md"), `Task Review repair ${ordinal}\n`);
        completeTaskStageHandoff(repair, {
          version: 1,
          stepId: "task-repair",
          completionStatus: "done",
          issues: [],
          ...
          repair: {
            version: 1,
            findings: [{ findingKey: recurringFinding.findingKey, paths: ["README.md"] }],
            summary: "Applied the exact round-two Task Review correction.",
            recurrenceResolutions: recurrence.entries.map(({ findingKey, fingerprint }) => ({
              findingKey,
              fingerprint,
              priorRepairInsufficiency: "The prior change did not fully cover the exact recurring behavior.",
              repairStrategy: "Extend the same Task source path with the missing round-two behavior.",
            })),
          },
          noChangeReason: null,
        });
      ...
      const secondReview = await publishTaskReview({ repository, manager, specId });
    ...
    const advisoryEligibility = definitionNonblockingEligibilityForActiveFlow(
      repository,
      advisoryManager.load(specId),
      advisoryManager,
    );
    assert.equal(
      resolveGateTransition(advisoryEligibility.gateDecision.facts).plan.action.identity
        .matches(advisoryEligibility.gateDecision.plan.action.identity),
      true,
    );
    ...
    assert.equal(findings.entries[0].attempts, 1);
  ...
  it("repairs a Task Gate only from its current receipt and sealed lifecycle plan", async () => {
    ...
    const coordinator = new WorkerArtifactHandoffCoordinator();
    coordinator.startSourceWorker({ ctx: context, request: handoff, invocation: {
      id: "task-gate-repair-worker",
      target: { digest: "b".repeat(64) },
      action: {
        digest: "a".repeat(64),
    ...
    materializeSourceWorkerEffect({
      ...
      responseText: JSON.stringify({
        ...
        gateRepair: {
          version: 1,
          summary: "The attempted repair produced no source mutation.",
          results: canonicalRepair.observationRequests.map((request) => ({
            fingerprint: request.fingerprint.toString(),
    ...
    const persistedOutcome = manager.readArtifact({
      specId,
      logicalKey: "plan.gate.repair.outcome",
      parameters: { repairId: canonicalRepair.idempotencyKey },
      consumerNodeId: "system",
    });
    assert.equal(JSON.parse(persistedOutcome.bytes).disposition, "rejected-no-progress");
    assert.equal(manager.activityLedger(specId).at(-1).transition.operation, "fail_attempt");
    const status = new GetStatusCommand().execute({
      ...context,
      flowState: manager.load(specId),
    });
  ...
  it("publishes Task Review must-fix findings for sealed triage without repairing source", async () => {
    const repository = root();
    initializeReviewSource(repository);
    const manager = new FlowManager({ root: repository, mainRoot: repository, inWorktree: false });
    ...
    await FLOW_COMMANDS.run.review.post(ctx, result);
    assert.equal(manager.artifactCatalog(specId).artifacts.some((artifact) => artifact.logicalKey === "task.review"), true);
    assert.equal(manager.canonicalState(specId).current.at(-1), "T-1-triage");
    assert.deepEqual(manager.taskMutationLineages({ specId, taskId: "T-1" }).map((lineage) => lineage.role), ["implementation"]);
  });
