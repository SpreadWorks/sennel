    ...
    const handoffInstruction = this.handoffRequest
      ? this.handoffRequest.policy.kind === "source"
        ...
        : [
          ...
          "For a source effects.json, every files[].requirementId must be copied from",
          "an exact requirement.id in the supplied spec. Guardrail IDs and finding",
          "requirementRef values (for example, no-overengineering) are not spec",
          "requirement IDs and must never be used as files[].requirementId.",
          "",
