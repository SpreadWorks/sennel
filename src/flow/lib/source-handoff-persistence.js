...
/**
 * Terminal source handoff authority needed only to discard its consumed
 * execution capability. The settlement, checkpoint envelope, and event head
 * remain verified without reopening the checkpoint's retired snapshot body.
 */
export class CanonicalSettledSourceHandoffAuthority {
  constructor({ identity, checkpointDigest, event, settlement } = {}) {
    if (!(identity instanceof SourceWorkerHandoffIdentity)) {
      throw new CurrentFlowStateInvariantError("settled source handoff authority requires a typed identity");
    }
    if (!(event instanceof SourceHandoffEvent) || !(settlement instanceof SourceHandoffSettlement)) {
      throw new CurrentFlowStateInvariantError("settled source handoff authority requires its typed event and settlement");
    }
    assertSameIdentity(identity, event.identity, "settled source handoff event");
    assertSameIdentity(identity, settlement.identity, "settled source handoff settlement");
    if (event.checkpointDigest !== checkpointDigest
      || settlement.checkpointDigest !== checkpointDigest
      || settlement.eventDigest !== event.digest) {
      throw new CurrentFlowStateInvariantError("settled source handoff authority does not bind its checkpoint and event head");
    }
    assertSourceHandoffSettlementTransition(event, settlement);
...
/** Read terminal authority without interpreting a retired checkpoint body. */
export function readSettledSourceHandoffAuthoritiesFromView({ view } = {}) {
  const authorities = view.catalog.artifacts
    .filter((entry) => entry.logicalKey === SETTLEMENT_KEY)
    .map((settlementDescriptor) => {
      const settlement = SourceHandoffSettlement.fromStored(
        parseJSON(view.readCatalogedArtifact(settlementDescriptor), "source handoff settlement"),
      );
      const identity = settlement.identity;
      const checkpointPath = FLOW_ARTIFACT_CONTRACTS.resolve(CHECKPOINT_KEY, { handoffId: identity.storageId }).relativePath;
      const checkpointDescriptor = view.catalog.artifacts.find((entry) => (
        entry.logicalKey === CHECKPOINT_KEY && entry.relativePath === checkpointPath
      )) ?? null;
      if (checkpointDescriptor === null) {
        throw new CurrentFlowStateInvariantError("settled source handoff checkpoint is absent from the catalog");
      }
      const checkpointDocument = parseJSON(
        view.readCatalogedArtifact(checkpointDescriptor),
        "settled source handoff checkpoint",
      );
      const checkpointIdentity = new SourceWorkerHandoffIdentity(checkpointDocument.identity);
      assertSameIdentity(identity, checkpointIdentity, "settled source handoff checkpoint");
      if (checkpointDocument.digest !== settlement.checkpointDigest) {
        throw new CurrentFlowStateInvariantError("settled source handoff checkpoint does not bind its settlement");
      }
      const matchingEvents = view.catalog.artifacts.filter((entry) => (
        entry.logicalKey === EVENT_KEY
        && entry.relativePath.startsWith(eventPrefix(identity))
        && entry.relativePath.endsWith(`-${settlement.eventDigest}.json`)
      ));
      if (matchingEvents.length !== 1) {
        throw new CurrentFlowStateInvariantError("settled source handoff event head is absent from the catalog");
      }
      const event = SourceHandoffEvent.fromStored(
        parseJSON(view.readCatalogedArtifact(matchingEvents[0]), "settled source handoff event"),
      );
      return new CanonicalSettledSourceHandoffAuthority({
        identity,
        checkpointDigest: settlement.checkpointDigest,
        event,
        settlement,
      });
    });
  if (new Set(authorities.map((entry) => entry.identity.storageId)).size !== authorities.length) {
    throw new CurrentFlowStateInvariantError("settled source handoff authority is duplicated");
  }
