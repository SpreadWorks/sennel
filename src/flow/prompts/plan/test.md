   ...
   - Work only on the Requirement selected by the handoff's `requirementTestBinding`.
   - Treat its Requirement id, approved Spec revision, bundle revision, and source Attempt as immutable capabilities. Read the assigned Requirement's expectation from the immutable `spec.json` handoff input. Stop if any value is absent or stale.
   ...
   - Every candidate file must use the canonical `// spec: R1 R2 ...` (or supported comment equivalent) header. The assigned Requirement must have exactly one named `R1:`-style test across the candidate. Secondary Requirement headers may be preserved but do not satisfy this work item.
