...
- Every finding is preserved for the host filter. Findings are selected for Task repair unless the host excludes their stable findingId with a non-empty canonical reason; classification alone does not remove a finding from repair.
...
- If a proposal concerns an intentional guardrail exception, record the applicable guardrail evidence in the finding rationale. Do not record or remediate an exception during review; the host filter decides whether an existing acknowledged exception applies.
- **If `impl-review.json.verdict` is `REJECTED`**:
  1. Read `impl-review.json` and `review.md`.
  2. Report each finding exactly as observed. Do not correct source or explain how a prior correction was insufficient; the host filter decides exclusions and task-repair owns source changes and repair evidence.
...
- **If the verdict is `PASS` or no findings were detected**: Display "レビューの結果、修正の必要はありませんでした。"
...
- When the verdict is `PASS` or the result contains no findings, follow the Definition-selected no-finding continuation (normally `task-gate`, or the canonical no-change completion when applicable). When the result contains one or more findings—including an `ADVISORY` result—the next-action CLI advances to the typed host filter boundary regardless of their disposition.
